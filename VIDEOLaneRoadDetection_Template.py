...
#dirVideo="VID_GoodVisiBility.mp4"
# needs a new template
#dirVideo="VID_badlane.mp4"
MaxAttempts=2
...
template = cv.imread('TemplateLaneRoad1.jpg', cv.IMREAD_GRAYSCALE)
assert template is not None, "file could not be read, check with os.path.exists()"
w, h = template.shape[::-1]

templateWorn = cv.imread('TemplateWornLaneRoad1010.jpg', cv.IMREAD_GRAYSCALE)
assert templateWorn is not None, "file could not be read, check with os.path.exists()"
wWorn, hWorn = templateWorn.shape[::-1]
...
while (cap.isOpened()):
        ...
        if ret != True: break
            ...
            while cont==0:
                ...
                # from https://stackoverflow.com/questions/59923076/how-to-automatically-adjust-the-threshold-for-template-matching-with-opencv
                # answer 0. good results but false hits
            ...
            if cont > 0:
                ...
                HitLanes=HitLanes+1
                    ...
                    # try templateWorn
                    res = cv.matchTemplate(img_gray,templateWorn,cv.TM_CCOEFF_NORMED)
                    cont=0
                    contAttempts=0
                    while cont==0:
                        #print("threshold = " + str(threshold))
                        ...
                        for pt in zip(*loc[::-1]):
                             ...
                             cv.rectangle(img_rgb, pt, (pt[0] + w, pt[1] + h), (255,0,0), 2)
                    if cont > 0:
                        print("Xtarget worn= " + str(pt[0]) + " Ytarget worn= " + str(pt[1]))
                        HitLanes=HitLanes+1
                    else:
